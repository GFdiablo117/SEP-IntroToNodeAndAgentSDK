...
class GreetingBot {
     ...
     */
    async start() {
        if (!this.core) this.core = new Agent({ accountId: this.accountId, username: this.username, password: this.password, 'csdsDomain': 'hc1n.dev.lprnd.net' });
     ...
     */
    async sendMessage(conversationId, message = `I'm the greeting bot and I like to welcome you here. An agent will be connected with you soon.`) {
        if (!this.isConnected) return;
...
console.log('Initializing the greeting bot...');
let bot = new GreetingBot(); // This will use the values set in the process.env
console.log('Starting the greeting bot...');
bot.start()
    .then(_ => console.log('Greeting bot is now up an running!'));